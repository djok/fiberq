...
must_haves:
  ...
  key_links:
    ...
    - from: "web/src/auth.ts"
      to: "server/api/auth/routes.py"
      via: "fire-and-forget fetch to POST /auth/record-login in jwt callback"
      pattern: "fetch.*record-login"
---
...
<task type="auto">
  <name>Task 3: Last login tracking in PostgreSQL and Next.js auth callback wiring</name>
  <files>
    server/db/init.sql
    server/api/auth/routes.py
    web/src/auth.ts
  ...
  <action>
    ...
    **2. Update `server/api/auth/routes.py`** -- add a POST /auth/record-login endpoint:
    ...
    Import `get_pool` from `database` if not already imported.

    **3. Update `web/src/auth.ts`** -- wire the jwt callback to call the record-login endpoint on initial sign-in:

    Read the existing file first. In the `jwt` callback, inside the `if (account)` block (which runs only on initial sign-in), add a fire-and-forget fetch to the record-login endpoint AFTER the existing role extraction logic. The access_token from the account object is the raw Kanidm JWT that FastAPI can validate.

    Add this at the end of the `if (account)` block (after `token.roles = roles;`):

    ```typescript
    // Record login event for last-login tracking (fire-and-forget)
    fetch(`${process.env.API_URL}/auth/record-login`, {
      method: "POST",
      headers: { Authorization: `Bearer ${account.access_token}` },
    }).catch(() => {}); // Don't block sign-in if this fails
    ```

    `process.env.API_URL` is the FastAPI server URL (already used by the existing `apiFetch` utility in `web/src/lib/api.ts`). Check what env var name is used in that file and use the same one. If the env var is different (e.g., `NEXT_PUBLIC_API_URL` or `API_BASE_URL`), use whichever is available in the server-side context (auth.ts runs server-side in Next.js).

    IMPORTANT: This must be fire-and-forget. Do NOT await the fetch. Do NOT let a failure block or break the sign-in flow. The `.catch(() => {})` ensures any network error is silently swallowed.
  ...
  <verify>
    ...
    grep -q "record-login" web/src/auth.ts && echo "auth.ts calls record-login"
  </verify>
  <done>user_logins table defined in init.sql. POST /auth/record-login endpoint exists and records user_sub, username, last_login_at, login_source with upsert logic. web/src/auth.ts jwt callback fires a POST to /auth/record-login on initial sign-in (fire-and-forget, non-blocking).</done>
</task>
...
6. web/src/auth.ts contains fetch to record-login (grep confirms)
</verification>
...
- web/src/auth.ts jwt callback calls record-login on sign-in (fire-and-forget)
