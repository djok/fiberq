...
must_haves:
  ...
  artifacts:
    - path: "web/src/types/project.ts"
      provides: "ProjectCard (with memberNames), CreateProjectInput TypeScript types"
    ...
    - path: "web/src/app/[locale]/projects/_components/projects-client.tsx"
      provides: "Client wrapper with card grid, filters (name, status, user), create button, and empty state"
      contains: "userFilter"
...
1. **Replace web/src/app/[locale]/projects/page.tsx** -- Server Component following Phase 2 users page pattern:
   ...
   - Define `ApiProject` type matching backend response (id, name, description, status, member_count, member_names, extent, created_at)
   - Define `mapProject()` function converting snake_case to camelCase (member_count -> memberCount, member_names -> memberNames, created_at -> createdAt)
   - Fetch projects via `apiFetch<ApiProject[]>("/projects")` with try/catch (empty array on failure)
   - Get user session for role check. Derive `canCreate = session.is_admin || session.roles.includes("project_manager")` per locked decision: "Admin + PM (global) can create projects"
   - Render `<ProjectsClient projects={projects} locale={locale} canCreate={canCreate} />`

2. **Create web/src/app/[locale]/projects/_components/projects-client.tsx** -- "use client" wrapper:
   - Props: `{ projects: ProjectCard[], locale: string, canCreate: boolean }`
   - State: `searchQuery` (string), `statusFilter` (string | "all"), `userFilter` (string | "all"), `showCreateDialog` (boolean)
   - Use `useTranslations("projects")` for i18n
   - **Derive unique user names for filter dropdown:** `const allMemberNames = [...new Set(projects.flatMap(p => p.memberNames))].sort()` -- this builds the list of available users to filter by from the memberNames field returned by the backend
   - **Header row:** Title on left, "Create Project" button on right (visible only when `canCreate` is true, covering both admin and global PM per locked decision)
   - **Filter bar:**
     ...
     - Select dropdown for user filter with options: "All users" (value "all") + one option per unique member name from `allMemberNames`. Label: t("filterByUser") for the "all" option.
   - **Card grid:** `grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4`
     - Filter projects by: (1) searchQuery (case-insensitive name match), (2) statusFilter, and (3) userFilter -- when userFilter is not "all", only show projects where `project.memberNames.includes(userFilter)`
     - Map filtered projects to `<ProjectCard />` components
   - **Empty state:** When no projects after filtering:
     - If `canCreate` and no projects at all: "No projects yet. Create your first project."
     - If not canCreate and no projects: "You have no assigned projects. Contact an administrator." (per user decision)
...
`grep 'ProjectsClient' web/src/app/[locale]/projects/page.tsx` -- client wrapper used.
  ...
  <done>
Project list page renders a responsive card grid (1/2/3 columns) with PostGIS mini-maps, status badges, member counts, and truncated descriptions. Filters by name, status, and assigned user work client-side (user filter uses memberNames from backend). Create dialog visible when canCreate=true (admin or global PM). Empty states differ for canCreate vs non-canCreate users. Cards link to detail page. Mini-maps use Intersection Observer for lazy loading to handle WebGL limits.
  </done>
</task>
...
<success_criteria>
Project list page shows a card grid with mini-maps, status badges, member counts, and three client-side filters (name search, status dropdown, assigned user dropdown). Admin or global PM (canCreate) can create projects via modal. Non-privileged users without projects see the correct empty state. Cards navigate to detail page on click.
</success_criteria>
