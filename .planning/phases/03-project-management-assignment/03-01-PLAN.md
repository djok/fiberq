...
must_haves:
  truths:
    - "GET /projects returns only assigned projects for non-admin/non-PM users, all projects for admin/PM"
    ...
    - "project_users table exists with project_role CHECK constraint and unique (project_id, user_sub)"
  artifacts:
    ...
    - path: "server/api/projects/models.py"
      provides: "Pydantic models for project CRUD with status, members, extent"
...
- `ProjectOut`: add `status: str`, `member_count: int = 0`, `extent: dict | None = None` fields
